...
describe('model', () => {
    describe('errors.ts', () => {
        ...
        describe('LoginNotFoundError', () => {
            it('should correctly set the properties', () => {
                const error = new errors.LoginNotPresentError(5, errors.LoginType.DEFAULT);

                expect(error.name).toBe('LOGIN_NOT_FOUND_ERROR');
                expect(error.message).toContain('5');
                expect(error.message).toContain(errors.LoginType.DEFAULT);
                expect(error.data.id).toBe(5);
                expect(error.data.type).toBe(errors.LoginType.DEFAULT);
                expect(error.httpCode).toBe(404);
            });
